...
@implementation SKImageScaleTool

+ (instancetype)sharedManager {
    
    static SKImageScaleTool *_instace = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _instace = [[SKImageScaleTool alloc] init];
    });
    return _instace;
}
...
+ (NSData *)compressOfImageWithWeiChat:(UIImage *)source_image
    ...
    if ([SKNetworkConfig sharedConfig].debugMode) {
        SKNetworkLog(@"原图大小: %lu Kb", (unsigned long)imageData.length/1024);
    }
    
    
    SKImageScaleTool *tool = [SKImageScaleTool sharedManager];
    ...
    if ([SKNetworkConfig sharedConfig].debugMode) {
        SKNetworkLog(@"压缩时间: %.4lf s,  压缩后图片大小: %lu Kb", linkTime, (unsigned long)imageData.length/1024);
    }
...
+ (NSData *)compressOfImageWithTwoPoints:(UIImage *)source_image maxSize:(NSInteger)maxSize {
    ...
    CFAbsoluteTime startTime = CFAbsoluteTimeGetCurrent();
    if ([SKNetworkConfig sharedConfig].debugMode) {
        SKNetworkLog(@"原图大小: %lu Kb", (unsigned long)finallImageData.length/1024);
    }
    ...
    CFAbsoluteTime linkTime = (CFAbsoluteTimeGetCurrent() - startTime);
    if ([SKNetworkConfig sharedConfig].debugMode) {
        SKNetworkLog(@"压缩时间: %.4lf s,  压缩后图片大小: %lu Kb", linkTime, (unsigned long)finallImageData.length/1024);
    }
...
- (NSData *)halfFuntion:(NSArray *)arr image:(UIImage *)image sourceData:(NSData *)finallImageData maxSize:(NSInteger)maxSize {
    ...
    while(start <= end) {
        ...
        //        SKNetworkLog(@"当前降到的质量 -> %ld 压缩系数 -> %lg", (unsigned long)sizeOriginKB,  [arr[index] floatValue]);
