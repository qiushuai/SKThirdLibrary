...
// 自定义滚动条
@property (nonatomic, strong) UIImageView *customLineImage;
// 滚动条
...
- (void)addScrollLineOrCoverOrExtraBtn {
    ...
    if (self.segmentStyle.isShowCustomLine) {
        self.segmentStyle.showLine = NO;
        [self.scrollView addSubview:self.customLineImage];
    }
...
- (void)setupScrollLineAndCover {
    ...
    if (self.customLineImage) {
        
        CGSize size = self.segmentStyle.customLine.size;
        if (self.segmentStyle.isScrollTitle && self.segmentStyle.customLine) {
            
            self.customLineImage.frame = CGRectMake(coverX + size.width, self.zj_height - size.height - self.segmentStyle.lineBottomHeight, size.width, size.height);
            ...
            self.customLineImage.frame = CGRectMake(coverW / 2 + size.width, self.zj_height - size.height - self.segmentStyle.lineBottomHeight, size.width , size.height);
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            self.scrollLine.frame = CGRectMake(coverX , self.zj_height - self.segmentStyle.scrollLineHeight - self.segmentStyle.lineBottomHeight, coverW , self.segmentStyle.scrollLineHeight);
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated taped:(BOOL)taped {
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        ...
        if (weakSelf.customLineImage) {
            
            CGFloat imageWidth = weakSelf.customLineImage.zj_width;
            if (weakSelf.segmentStyle.isScrollTitle) {
                weakSelf.customLineImage.zj_x = currentTitleView.zj_x + imageWidth;
            } else
            {
                if (weakSelf.segmentStyle.isAdjustCoverOrLineWidth) {
                    ...
                    weakSelf.customLineImage.zj_x = scrollLineX + imageWidth;
...
- (void)adjustUIWithProgress:(CGFloat)progress oldIndex:(NSInteger)oldIndex currentIndex:(NSInteger)currentIndex {
    ...
    if (self.customLineImage) {
        ...
        CGFloat imageWidth = self.customLineImage.zj_width;
        if (self.segmentStyle.isShowCustomLine && self.segmentStyle.customLine) {
            
            self.customLineImage.zj_x = oldTitleView.zj_x + xDistance * progress + imageWidth;
        ...
        } else
        {
            if (self.segmentStyle.isAdjustCoverOrLineWidth) {
                ...
                self.scrollLine.zj_x = oldScrollLineX + xDistance * progress + imageWidth;
                
            } else
            {
                self.scrollLine.zj_x = oldTitleView.zj_x + xDistance * progress + imageWidth;
            }
        }
...
- (UIImageView *)customLineImage {
    ...
    if (!_customLineImage) {
        ...
        lineView.image = self.segmentStyle.customLine;
        _customLineImage = lineView;
    }
    
    return _customLineImage;
}
